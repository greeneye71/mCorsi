...
class TemplateUploadForm(FlaskForm):
    ...
    file = FileField("Modello DOCX", validators=[FileRequired(), FileAllowed(["docx"])])
...
class SignatureUploadForm(FlaskForm):
    ...
    file = FileField("Firma PNG o JPEG", validators=[FileRequired(), FileAllowed(["png", "jpg", "jpeg"])])
...
class ParticipantCertificateUploadForm(FlaskForm):
    ...
    file = FileField("Attestato PDF", validators=[FileRequired(), FileAllowed(["pdf"])])
